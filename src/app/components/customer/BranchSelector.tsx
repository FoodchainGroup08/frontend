      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        <div className="flex items-center justify-between mb-8">
          <Link to="/" className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg" style={{ color: 'var(--espresso)', fontWeight: 600 }}>FoodChain</h2>
              {userName && (
        ...
        <div className="text-center mb-8 sm:mb-12">
          <h1 className="text-3xl sm:text-4xl mb-3" style={{ color: 'var(--espresso)', fontWeight: 600 }}>
            Select Your Branch
          </h1>
          {usingSource === 'saved' && (
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm mb-3" style={{ backgroundColor: 'var(--sage-green)', color: 'var(--white)' }}>
              <MapPin className="w-3.5 h-3.5" />
              Sorted by your saved address
            </div>
          )}
          {usingSource === 'gps' && (
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm mb-3" style={{ backgroundColor: 'var(--golden-amber)', color: 'var(--charcoal)' }}>
              <MapPin className="w-3.5 h-3.5" />
          ...
          <p className="text-base sm:text-lg" style={{ color: 'var(--espresso)', opacity: 0.7 }}>
        ...
        {isLoading ? (
          ...
          <div className="grid gap-4 sm:gap-6">
            {branches.map((branch, index) => {
              ...
              <Card
                key={branch.id}
                className="cursor-pointer transition-all hover:shadow-lg border-[var(--espresso)]/10"
                onClick={() => open && onSelectBranch(branch)}
                style={{
                  backgroundColor: 'var(--white)',
                  opacity: open ? 1 : 0.6,
                  borderColor: isSuggested ? 'var(--golden-amber)' : undefined,
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <CardTitle className="text-lg sm:text-xl" style={{ color: 'var(--espresso)' }}>
                          {branch.name}
                        </CardTitle>
                        {isSuggested && (
                          <Badge
                        ...
                        {open ? (
                          ...
                          <Badge className="border-0" style={{ backgroundColor: 'var(--burnt-orange)', color: 'var(--white)' }}>
                            Closed
                          </Badge>
                        )}
                      </div>
                      <div className="flex items-center gap-1 mb-2">
                        <Star className="w-4 h-4" style={{ color: 'var(--golden-amber)', fill: 'var(--golden-amber)' }} />
                        <span className="text-sm" style={{ color: 'var(--espresso)', fontWeight: 600 }}>
                          {getDisplayRating(branch.rating, branch.id)}
                        </span>
                      </div>
                    </div>
                    {branch.distance && (
                      <div className="text-right">
                        <p className="text-sm" style={{ color: 'var(--golden-amber)', fontWeight: 600 }}>
                          {branch.distance}
                        </p>
                      </div>
                    )}
                ...
                <CardContent>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      <MapPin className="w-4 h-4 mt-0.5 flex-shrink-0" style={{ color: 'var(--espresso)', opacity: 0.6 }} />
